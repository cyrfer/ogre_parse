...
class UnifyTechniques(object):
    ...
    # def bail(self, msg, mname):
    #     logtext = 'UnifyTechniques is bailing on Material [%s] because [%s].' % (mname, msg)
    #     logging.info(logtext)
    #     print(logtext)
    #
    # def unify_one(self, src):
    #     modded = False
    #     dest = copy.deepcopy(src)
    #     for itech in range(len(dest.techniques)):
    #         tdest = dest.techniques[itech]
    #         for pdest in tdest.passes:
    #             for sh in pdest.shaders:
    #                 if self.mod_shader(sh, itech, src.name):
    #                     modded = True
    #
    #     return modded, dest
    #
    # def mod_shader(self, sh, tech_index, mname):
    #     modded = False
    #     if sh.resource_name:
    #         if sh.resource_name not in self.unified:
    #             if sh.resource_name in self.unified.values():
    #                 self.bail('shader [%s] is already a unified resource in Technique [%s]' % (sh.resource_name, str(tech_index)), mname)
    #             else:
    #                 self.bail('shader [%s] is unknown in Technique [%s]' % (sh.resource_name, str(tech_index)), mname)
    #         else:
    #             # maybe it already uses a unified name
    #             if sh.resource_name not in self.unified.values():
    #                 sh.resource_name = self.unified[sh.resource_name]
    #                 modded = True
    #
    #     return modded
    #
    # def has_non_unified_shaders(self, tech):
    #     hasUnified = True
    ...
    #                 hasUnified = False
    #
    #     return not hasUnified
    #
    # # replaces first 2 techniques with a single technique using unified shader names
    # def unify_two(self, src):
    #     if len(src.techniques[0].passes) != len(src.techniques[1].passes):
    #         self.bail('its Techniques did not have the same number of passes', src.name)
    #         return None
    #
    #     # check if the techniques have good shaders
    #     hasUnknownShaders = False
    #     for tsrc in src.techniques[0:2]:
    #         if self.has_non_unified_shaders(tsrc):
    #             hasUnknownShaders = True
    #
    #     #     for psrc in tsrc.passes:
    #     #         for psh in psrc.shaders:
    #     #             # check if we have the right shaders
    #     #             if psh.resource_name not in self.unified:
    #     #                 # maybe it already uses unified names
    #     #                 if psh.resource_name not in self.unified.values():
    #     #                     self.bail('shader [%s] has no known unified equivalent' % psh.resource_name, src.name)
    #     #                     hasUnknownShaders = True
    #
    #     if hasUnknownShaders:
    #         self.bail('its passes use shaders with no known unified equivalent', src.name)
    #         return None
    #
    #     # ------ ready to transform the material ------ #
    #     # --- build the sole technique --- #
    #     # grab the properties
    #     tdest = copy.deepcopy(src.techniques[0])
    #
    #     # nuke the passes
    #     tdest.passes.clear()
    #
    #     # build the passes
    #     for ip in range(len(src.techniques[0].passes)):
    #         pdup0 = copy.deepcopy(src.techniques[0].passes[ip])
    #         pdup1 = copy.deepcopy(src.techniques[1].passes[ip])
    #
    #         # unify the shader names in pass0
    #         # TODO: maybe more readable with map()?
    #         for sh in pdup0.shaders:
    #             self.mod_shader(sh, 0, src.name)
    #
    #         # unify the shader names in pass1
    #         for sh in pdup1.shaders:
    #             self.mod_shader(sh, 1, src.name)
    #
    #         # a sanity check to confirm the unified shaders are the same in both passes
    #         unify0 = [x.resource_name for x in pdup0.shaders if x.resource_name]
    #         unify1 = [x.resource_name for x in pdup1.shaders if x.resource_name]
    #         unify0.sort()
    #         unify1.sort()
    #         if unify0 != unify1:
    #             self.bail('list of shader resource names were not the same', src.name)
    #             continue
    #
    #         # add the pass to the unified technique
    #         tdest.passes.append(pdup0)
    #
    #     if len(tdest.passes) == 0:
    #         return None
    #
    #     # grab the properties
    #     dest = copy.deepcopy(src)
    #
    #     # nuke the techniques
    #     dest.techniques.clear()
    #
    #     # save the one we want to return
    #     dest.techniques.append(tdest)
    #
    #     # add the other ones
    #     dest.techniques.extend(copy.deepcopy(src.techniques[2:]))
    #     return dest
